...
#if !defined(IGNORE_MYDBG_IMPLEMENTATION)

arena_logger_t *arena_logger_registry = NULL;

static arena_logger_t *arena_logger_find(arena_t *const arena)
{
...
void arena_logger_init(arena_t *const arena, const char *file, int line)
{
    if (arena_logger_find(arena)) return;
...
void arena_logger_log_alloc(arena_t *const arena, uint64_t size, const char *file, int line, const char *func)
{
    arena_logger_t *l = arena_logger_find(arena);
    if (!l) return;
...
#endif // !defined(IGNORE_MYDBG_IMPLEMENTATION)
