...
vec3f_t workbench_editor_screen_to_ray_direction(const vec2f_t screen_pos)
{
    ...
    vec4f_t near = { screen_pos.x, screen_pos.y, -1.0f, 1.0f };
    vec4f_t far  = { screen_pos.x, screen_pos.y,  1.0f, 1.0f };
    ...
    return glms_vec3_normalize(glms_vec3_sub(*(vec3f_t *)&far_w, *(vec3f_t *)&near_w));
}

void workbench_editor_pick_entity(void)
{
    vec2f_t ndc = window_mouse_get_norm_position(global_window);
    vec3f_t dir = workbench_editor_screen_to_ray_direction(ndc);
    glcamera_t *cam = ecs_get_active_camera(global_ecs);
    JPH_RayCastResult hit = physics_sys_jolt_raycast(cam->position, dir);
    if (hit.bodyID) {
        global_workbench->selected_entity_id = ecs_entity_get_id_by_body_id(hit.bodyID);
    } else {
        global_workbench->selected_entity_id = 0;
    }
}
